        ...
        {orders.map((order) => (
          <article className={`order-history-card currency-history-card ${order.currencyType.toLowerCase()}`} key={order.id}>
            ...
            <div className="order-history-card-body currency-history-body">
              <div><small>Loại</small><strong className={`currency-history-type-text ${order.currencyType.toLowerCase()}`}>{order.currencyType === "GOLD" ? goldSaleTypeLabel(order.goldSaleType) : "Ngọc"}</strong></div>
              ...
              <div><small>Thực nhận</small><strong className="currency-history-received">{formatReceivedCurrency(order.receivedAmount, order.currencyType, order.goldSaleType)}</strong></div>
