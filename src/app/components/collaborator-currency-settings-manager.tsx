...
type ServerForm = { name: string; goldEnabled: boolean; goldAmount: string; goldPrice: string; goldSaleType: "BAR" | "FRESH"; gemEnabled: boolean; gemAmount: string; gemPrice: string; displayOrder: string; toolServerIndex: string; active: boolean };
const emptyForm: ServerForm = { name: "", goldEnabled: true, goldAmount: "37000000", goldPrice: "10000", goldSaleType: "BAR", gemEnabled: false, gemAmount: "100", gemPrice: "10000", displayOrder: "0", toolServerIndex: "1", active: true };
...
export function CollaboratorCurrencySettingsManager() {
  ...
  function openEdit(config: CollaboratorCurrencyServer) {
    ...
    setForm({ name: config.name, goldEnabled: config.goldEnabled && !!policy?.goldSellingEnabled, goldAmount: String(config.goldAmount), goldPrice: String(config.goldPrice), goldSaleType: config.goldSaleType ?? "BAR", gemEnabled: config.gemEnabled && !!policy?.gemSellingEnabled, gemAmount: String(config.gemAmount), gemPrice: String(config.gemPrice), displayOrder: String(config.displayOrder), toolServerIndex: String(config.toolServerIndex), active: config.active });
    setMessage(""); setError(""); setIsModalOpen(true);
  }
...
function nextAvailableToolIndex(configs: CollaboratorCurrencyServer[]) { const used = new Set(configs.map((config) => config.toolServerIndex)); for (let index = 1; index <= 22; index += 1) if (!used.has(index)) return index; return 22; }
